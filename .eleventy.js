...
const api = new ghostContentAPI({
    ...
    key: "3eadeb0d50b3b85c28a29540e6",
    version: "v2",
});

// Strip Ghost domain from urls
const stripDomain = (url) => {
    return url.replace("https://komodoplatform.com/en/blog", "");
};
...
module.exports = function (config) {
    ...
    config.addPlugin(lazyImages, {
        ...
        // transformImgPath: (path) => {
        //     if (path.startsWith("./en/blog")) {
        //         path = path.slice(1);
        //         return "https://komodoplatform.com" + path;
        //     } else {
        //         return path;
        //     }
        // },
    ...
    config.addCollection("posts", async function (collection) {
        ...
        collection.forEach((post) => {
            ...
            let filteredTags = [];
            tags.forEach((tag) => {
                let strippedTagURL = stripDomain(tag.url);
                if (strippedTagURL !== "/404/") {
                    filteredTags.push(strippedTagURL);
                }
            });
