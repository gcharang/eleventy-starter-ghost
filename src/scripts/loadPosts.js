...
function generateCardHTML(post){
    ...
    cardHTML = `<a class="post-card ${ post.featured? "post-card-featured" : ""}" href="${ post.url }">`
                + header + excerpt + footer + `</a>`;
    return cardHTML;
}
...
async function loadMorePosts(){
    ...
    let posts = [];
    try{
        let response = await fetch(`/data/posts/allPosts/${postSetLoadCount}.json`);                
